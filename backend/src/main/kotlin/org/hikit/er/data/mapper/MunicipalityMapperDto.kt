...
@Component
class MunicipalityMapperDto {
    fun map(localityDto: MunicipalityDto): Municipality {
        throw NotImplementedError()
    }

    fun map(lc: Municipality): MunicipalityDto =
        MunicipalityDto(
            lc.remoteId,
            lc.name,
            lc.description,
            ...
            geometry = lc.geometry.map { CoordinatesDto(it.latitude, it.longitude) }
        )
}
